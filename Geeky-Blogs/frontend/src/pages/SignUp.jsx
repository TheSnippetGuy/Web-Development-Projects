...
function SignUp() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    let form = new FormData(signUpForm.current);
    let formData = {};
    ...
    let { email, fullName, password, confirmPassword } = formData;
    ...
    try {
      let response = await axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/user/signup", formData);
  ...
  return accessToken ? (
    ...
    <PageAnimation>
      <section className="h-cover flex items-center justify-center">
        <form className="w-[80%] max-w-[400px]" ref={signUpForm}>
          <h1 className="text-3xl font-gelasio capitalize text-center mb-24 font-semibold">Join Us Today</h1>
          <InputBox name="fullName" type="text" placeholder="Your Full Name" id="fulName" icon="user" />
